...
def h(a, b):
    ...
    #Outputs: value of heuristic function
...
def printMaze():
    global maze
    for row in maze:
        #Print each column in a row on one line
        for col in row:
            print(col, end = " ")
        print()
...
def printMazeWithCosts():
    ...
    for y in range(height):
        for x in range(width):
            if maze[y][x] == searched_:
                print(Back.BLUE + str(mazeCosts[y][x]), end = " ")
            elif maze[y][x] == wall_:
                print(wall_, end = " ")
            elif maze[y][x] == cell_:
                print(Back.BLACK + str(mazeCosts[y][x]), end = " ")
            elif maze[y][x] == path_:
                print(Back.CYAN + str(mazeCosts[y][x]), end = " ")
        print()
    print(Style.RESET_ALL)
...
#Random start space in top row -> (x, 0)
#Make sure not to include left and right-most columns
...
#Initialize array for walls that could be changed to a cell
walls = [start]

#Carve path through maze, based on Prim's Algorithm
#Loop while walls has a space to check in it
while walls:
    #Remove wall from list and save it for checking
    tile = walls.pop( random.randint(0, len(walls)-1) )
    #Get nearby walls from this tile
    nearbyWalls = getNearbyWalls(tile)
    
    #Tile has 3-4 nearby walls, thus can be made a cell
    if len(nearbyWalls) >= 3:
        #For each nearby wall, add it to walls array to check later
        for wall in getNearbyWalls(tile):
            walls.append(wall)
        #Set point (x, y) to a cell; maze[y][x] is point (x, y)
...
for x in range(width):
    for y in range(height):
        ...
        if len(nearbyWalls) == 2:
            ...
            #If these nearby walls are on opposite sides of checked wall
            #   remove the checked wall, forming a "hallway"
...
#3D array containing attempted paths to goal
paths = [[(start)]]
...
#3D array containing attempted paths to goal and their cost
